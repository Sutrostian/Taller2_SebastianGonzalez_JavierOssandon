...
public class Partida {
	...
	public void ListaPokemons() {
		for (int i = 0; i < EquipoPokemon.size(); i++) {
			...
			System.out.println((i + 1) + ".-" + pokemon.getNombrePokemon());
	...
	public boolean SimularCombate(String Enemigo, ArrayList<Pokemon> EquipoPokemon, ArrayList<Pokemon> Equipoenemigo,
		...
		System.out.println(NombreJugador + " Saca a " + PokemonActual.getNombrePokemon() + "Stats :"
				+ PokemonActual.getEstadisticasTotales());
		...
		while (!AcaboElCombate) {
			...
			case "1":
				...
				System.out
						.println(PokemonEnemigo.getNombrePokemon() + "--->" + PokemonEnemigo.getEstadisticasTotales());
				...
				double NuevaEstadisticasAliado = EstadisticasAliado
						* TablaTipos.calcularEfectividad(PokemonActual, PokemonEnemigo);
				double NuevaEstadisticasEnemigo = EstadisticasEnemigo
						* TablaTipos.calcularEfectividad(PokemonEnemigo, PokemonActual);

				System.out.println(
						"Nuevas Estadisticas " + PokemonActual.getNombrePokemon() + "--->" + NuevaEstadisticasAliado);
				System.out.println(
						"Nuevas Estadisticas " + PokemonEnemigo.getNombrePokemon() + "--->" + NuevaEstadisticasEnemigo);
				...
				if (NuevaEstadisticasAliado < NuevaEstadisticasEnemigo) {
					...
					PokemonActual.setEstado("Debilitado");
					...
					while (!RemplazoValido) {
						...
						if (vivos == 0) {
							System.out.println("Todos Tus Pokemons Han Sido Debilitados, Has Sido Derrotado");
							return false;
						}
						...
						try {
							...
							pos = Integer.parseInt(EleccionRemplazo) - 1;
						...
						if (pos < 0 || pos >= 6 || EquipoPokemonActual[pos] == null) {

							System.out.println("Posicion invalida");
							continue;
						}

						if (EquipoPokemonActual[pos].getEstado().equals("Vivo")) {

							PokemonActual = EquipoPokemonActual[pos];
							System.out.println(NombreJugador + " Saca a " + PokemonActual.getNombrePokemon() + "Stats: "
									+ PokemonActual.getEstadisticasTotales());
							System.out.println(Enemigo + " Tiene a " + PokemonEnemigo);
							RemplazoValido = true;
						}
						if (EquipoPokemonActual[pos].getEstado().equals("Debilitado")) {
			...
			case "2":
				...
				try {
					...
					pos = Integer.parseInt(EleccionCombate) - 1;
			...
			case "3":
				...
				try {
					...
					Rendirse = Integer.parseInt(s.nextLine());
	...
	public boolean TienePokemonVivos() {
		...
		for (Pokemon p : EquipoPokemonActual) {
	...
	public ArrayList<Pokemon> getTodosLosPokemons() {
